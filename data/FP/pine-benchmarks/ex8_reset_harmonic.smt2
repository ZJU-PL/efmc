; ex8_reset_harmonic.txt converted to CHC format
; Original: Conditional discrete-time system with reset
; State: x0, x1
; Input: r ∈ [0.0, 1.0]
; Initial: x0, x1 ∈ [0.0, 1.0], r ∈ [0.0, 1.0]
; Transition: if r > 0.5 then {x0' = 0.95*x0 + 0.09975*x1, x1' = -0.1*x0 + 0.95*x1}
;             else {x0' = 1, x1' = 1}
...
; Initial state: x0, x1 ∈ [0.0, 1.0], r ∈ [0.0, 1.0]
...
; If r > 0.5: x0' = 0.95*x0 + 0.09975*x1, x1' = -0.1*x0 + 0.95*x1
; Else: x0' = 1, x1' = 1
...
(assert (forall ((x0 (_ FloatingPoint 8 24)) (x1 (_ FloatingPoint 8 24)) (r (_ FloatingPoint 8 24))
         ...
         (=> (and (inv x0 x1 r)
                  ...
                  (ite (fp.gt r ((_ to_fp 8 24) RNE 0.5))
                       (and (fp.eq x0! (fp.add RNE (fp.mul RNE ((_ to_fp 8 24) RNE 0.95) x0)
