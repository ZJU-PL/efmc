; ex7_dampened.txt converted to CHC format
; Original: Dampened discrete-time system
; State: x0, x1
; Initial: x0, x1 ∈ [0.0, 1.0]
; Transition: x0' = x0 + 0.01*x1
;            x1' = -0.1*x0 + 0.99*x1
...
; Transition relation: x0' = x0 + 0.01*x1, x1' = -0.1*x0 + 0.99*x1
(assert (forall ((x0 (_ FloatingPoint 8 24)) (x1 (_ FloatingPoint 8 24))
                 (x0! (_ FloatingPoint 8 24)) (x1! (_ FloatingPoint 8 24)))
         (=> (and (inv x0 x1)
                  (fp.eq x0! (fp.add RNE x0 (fp.mul RNE ((_ to_fp 8 24) RNE 0.01) x1)))
