...
class LLMInterface:
    ...
    def __init__(
        self, sts: TransitionSystem, model_name: str = "deepseek-v3", **kwargs
    ):
        self.sts = sts
    ...
    def generate_candidates(
        ...
        # Debug logging
        logger.info("LLM Response: %s...", resp[:200])
        ...
        try:
            ...
            logger.error("Response was: %s", resp)
            return []

    def call_llm(self, prompt: str, **kwargs) -> str:
        """Call LLM with the given prompt and return response."""
        self.stats["total_llm_calls"] += 1
        resp, _, _ = self.llm.infer(
            prompt, is_measure_cost=kwargs.get("measure_cost", False)
        )
        return resp
