...
class LLMInvariantCEGIS:
    ...
    def __init__(self, sts: TransitionSystem, **kwargs):
        ...
        self.max_candidates_per_iteration = kwargs.get(
    ...
    def _cegis_iteration(self, program_code: str) -> Optional[z3.ExprRef]:
        ...
        candidates = self.llm_interface.generate_candidates(
            prompt_generator=prompt_generator, response_parser=response_parser
        )

        self.stats["candidates"] += len(candidates)

        for cand_str, cand_expr in candidates:
            is_correct, counterexamples = verify_invariant_with_counterexamples(
                self.sts, cand_expr
            )
            if is_correct:
                return cand_expr
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        if self.stats["candidates"] > 0:
            stats["candidate_success_rate"] = (
                self.stats["successful"] / self.stats["candidates"]
            )
        return stats
