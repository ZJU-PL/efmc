    ...
    for ind, arg in enumerate(p):
        if isinstance(arg, str):
            ...
            for nsplit, subp in enumerate(arg):
                ...
                ssaed_subpath = nd_bb_path_to_ssa(
                    subp, bbs, SSAEnv(ssa_env, suffix), cur_p + suffix
                )
            ...
            for nsplit, (subp, sub_env) in enumerate(tmp):
                ...
                for s in changed:
                    ...
                    bb_stmts.append(
                        AstAssignment(AstId(ssa_env.lookup(s)), AstId(old_var))
                    )
...
def ssa_stmt(stmt, prev_replm, cur_replm):
    ...
    if isinstance(stmt, AstAssignment):
        return AstAssignment(
            replace(stmt.lhs, cur_replm), replace(stmt.rhs, prev_replm)
        )
    else:
...
def ssa_path_to_z3(ssa_path, bbs):
    def f(arg):
        if arg[0].startswith("_split_"):
            ...
            return Or(
                [
                    And((Int(split_var) == ind), ssa_path_to_z3(x, bbs))
...
def extract_ssa_path_vars(ssa_p, m):
    argsS = set(
        [str(x) for x in m if (not is_ssa_str(str(x)) and "_split_" not in str(x))]
    )
        ...
        for _, arg in enumerate(ssa_p):
            if arg[0].startswith("_split_"):
                ...
                for repl_m in repl_ms:
                    ...
                    new_env = {
                        orig_name: m.get(ssa_name, None)
                        for (orig_name, ssa_name) in [
                            (x, str(repl_m.get(AstId(x), x))) for x in vs
                        ]
                    }
                    envs.append(new_env)
