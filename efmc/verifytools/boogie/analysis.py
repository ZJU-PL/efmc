...
def livevars(bbs):
    transformer_m = {
        ...
        AstAssignment: lambda stmt, inS: (inS - stmt_changed(stmt)).union(
            stmt_read(stmt)
        ),
    }
...
def propagate_sp(bbs):
    ...
    def untouched(preds, clobbered_vars):
        return set(
            [x for x in preds if len(clobbered_vars.intersection(expr_read(x))) == 0]
        )

    def assignment_preds(stmt):
        return (
            set([AstBinExpr(stmt.lhs, "==", stmt.rhs)])
            if str(stmt.lhs) not in expr_read(stmt.rhs)
            else set()
        )
    ...
    transformer_m = {
        ...
        AstAssignment: lambda stmt, inS: untouched(inS, set([str(stmt.lhs)])).union(
            assignment_preds(stmt)
        ),
    }
