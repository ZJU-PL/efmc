                    ...
                    if succ in cps:
                        ...
                        for candidate in candidate_invs:
                            candidateSSA = expr_to_z3(
                                replace(candidate, curFinalSSAEnv.replm()), aiTyEnv
                            )
                            try:
                                c = counterex(
                        ...
                        succSSA, nextFinalSSAEnv = nd_bb_path_to_ssa(
                            [succ], bbs, SSAEnv(curFinalSSAEnv)
                        )
    ...
    for cp in cps:
        ...
        while len(workQ) > 0:
            ...
            if len(bbs[nextBB].successors) == 0:  # This is exit
                ...
                if c != None:
                    v = Violation(
                        "safety", path, processedStmts, Implies(sp, postSSA), c
                    )
                    violations.append(v)
            else:
                for succ in bbs[nextBB].successors:
