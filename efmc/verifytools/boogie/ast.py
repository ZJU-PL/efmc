...
class AstProgram(AstNode):
    def __init__(s, decls):
        AstNode.__init__(s, decls)
...
class AstBinding(AstNode):
    def __init__(s, names, typ):
        AstNode.__init__(s, names, typ)

    def __str__(s):
        return ",".join(map(str, s.names)) + " : " + str(s.typ)


class AstLabel(AstNode):
    def __init__(s, label, stmt):
        AstNode.__init__(s, label, stmt)

    def __str__(s):
        return str(s.label) + " : " + str(s.stmt)


class AstAssignment(AstNode):
    def __init__(s, lhs, rhs):
        AstNode.__init__(s, lhs, rhs)
...
class AstBody(AstNode):
    def __init__(s, bindings, stmts):
        AstNode.__init__(s, bindings, stmts)
...
class AstOneExprStmt(AstStmt):
    def __init__(s, expr):
        AstStmt.__init__(s, expr)


class AstAssert(AstOneExprStmt):
    def __str__(s):
        return "assert (" + str(s.expr) + ");"


class AstAssume(AstOneExprStmt):
    def __str__(s):
        return "assume (" + str(s.expr) + ");"


class AstHavoc(AstStmt):
    def __init__(s, ids):
        AstStmt.__init__(s, ids)

    def __str__(s):
        return "havoc " + ",".join(map(str, s.ids)) + ";"
...
class AstGoto(AstStmt):
    def __init__(s, labels):
        AstStmt.__init__(s, labels)

    def __str__(s):
        return "goto " + ",".join(map(str, s.labels)) + ";"
...
class AstWhile(AstStmt):
    ...
    def __str__(s):
        inv_str = (
            "\n".join([f"  invariant {inv};" for inv in s.invariants])
            if s.invariants
            else ""
        )
...
class AstBlock(AstStmt):
    def __init__(s, stmts):
        AstStmt.__init__(s, stmts)
...
class AstFuncExpr(AstNode):
    def __init__(s, funcName, *ops):
        AstNode.__init__(s, funcName, *ops)
...
class AstBuilder(BoogieParser):
    ...
    def onLABinOp(s, prod, st, loc, toks):
        if len(toks) == 3:
            ...
            rest = [
                [toks[3 + 2 * k], toks[3 + 2 * k + 1]]
                for k in range((len(toks) - 3) / 2)
            ]
    ...
    def onRABinOp(s, prod, st, loc, toks):
        if len(toks) == 3:
            ...
            base = AstBinExpr(*toks[:3])
            return [
                reduce(lambda acc, el: AstBinExpr(acc, el[0], el[1]), toks[3:], base)
            ]
...
def ast_and(exprs):
    return ast_group_bin(list(exprs), "&&", AstTrue())


def ast_or(exprs):
    return ast_group_bin(list(exprs), "||", AstFalse())
...
def normalize(ast):
    ...
    if (
        isinstance(ast, AstBinExpr)
        and ast.op == "||"
        and isinstance(ast.lhs, AstUnExpr)
        and ast.lhs.op == "!"
    ):
