...
class BoogieParser(InfixExprParser):
    ...
    def __init__(s):
        ...
        s.Expr << (
            s.BoolExpr ^ s.RelExpr ^ s.ArithExpr
        ...
        s.TypeCtorArgs = F()
        s.TypeCtorArgs = (
            s.TypeAtom + O(s.TypeCtorArgs) | s.Id + O(s.TypeCtorArgs) | s.MapType
        )

        s.Type << (
            s.TypeAtom | s.MapType | s.Id + O(s.TypeCtorArgs)
        ...
        s.ConstantDecl = (
            s.CONST + O(s.Attribute) + O(s.UNIQUE) + s.IdsType + s.OrderSpec
        )
        ...
        s.Spec = (
            O(s.FREE) + s.REQUIRES + s.Expr + s.SEMI
        ...
        s.PSig = (
            O(s.TypeArgs) + s.LPARN + csl(s.IdsTypeWhere) + s.RPARN + O(s.OutParameters)
        )

        s.LocalVarDecl = S(s.VAR) + ZoM(s.Attribute) + csl(s.IdsTypeWhere) + S(s.SEMI)
        s.LocalVarDecl.setParseAction(
            lambda st, loc, toks: s.onLocalVarDecl(s.LocalVarDecl, st, loc, toks)
        )
        ...
        s.AssignmentStmt.setParseAction(
            lambda st, loc, toks: s.onAssignment(s.AssignmentStmt, st, loc, toks)
        )
        ...
        s.CallAssignStmt = (
            s.CALL + O(s.CallLhs) + s.Id + s.LPARN + csl(s.Expr) + s.RPARN + S(s.SEMI)
        )
        s.CallForallStmt = (
            s.CALL
            + s.FORALL
        ...
        s.WhileStmt = (
            s.WHILE + s.LPARN + s.WildcardExpr + s.RPARN + ZoM(s.LoopInv) + s.BlockStmt
        )
        ...
        s.LEmpty = F()
        s.LEmpty << (
            s.Id + s.COLN + O(s.LEmpty)
        )  # pylint: disable=expression-not-assigned
        s.LabeledStatement.setParseAction(
            lambda st, loc, toks: s.onLabeledStatement(
                s.LabeledStatement, st, loc, toks
            )
        )

        s.StmtList << (
            ZoM(s.LStmt) + O(s.LEmpty)
        )  # pylint: disable=expression-not-assigned
