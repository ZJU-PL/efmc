...
class BoogieRuntimeExc(Exception):
    pass
...
if __name__ == "__main__":
    ...
    p.add_argument(
        ...
        choices=["all", "random_lookahead_1"],
        default="all",
    )
    ...
    starting_store = {
        k: int(v) for (k, v) in [x.split("=") for x in args.starting_env.split(",")]
    }
    ...
    def pp_state(st):
        return "[{},{}]: ".format(st.pc.bb, st.pc.next_stmt) + ",".join(
            [k + "={}".format(v) for (k, v) in st.store.items()]
        )
