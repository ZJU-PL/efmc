from efmc.verifytools.boogie.z3_embed import (
    And,
    stmt_to_z3,
    env_to_expr,
    satisfiable,
...
from efmc.verifytools.boogie.paths import (
    get_path_vars,
    nd_bb_path_to_ssa,
    sp_nd_ssa_path,
...
def evalPred(boogie_expr, env):
    ...
    q = And(
        map(
            lambda stmt: stmt_to_z3(stmt, typeEnv),
            [AstAssume(env_to_expr(env)), AstAssert(boogie_expr)],
        )
    )
    res = satisfiable(q)
    return res
...
def instantiateAndEval(inv, vals, var_names=None, const_names=None):
    ...
    typeEnv = {str(x) + str(i): Int for x in vals[0].keys() for i in range(len(vals))}
    ...
    for prm in prms:
        ...
        p += [
            AstAssert(
                replace(inst_inv, {AstId(x): AstId(x + str(i)) for x in varM.values()})
            )
...
def execute(env, bb, bbs, limit):
    ...
    while len(q) > 0:
        ...
        if len(bbs[bb].successors) == 0 or len(curp) + 1 >= limit:
            yield postcond, after_env, newp, new_ssap, extract_ssa_path_vars(
                new_ssap, model(postcond)
            )
            continue
